...
files_idir ='E:\DATA\5.TCdata\TCraster500km\';
raster_files = dir([files_idir,'*.txt']);
PERSIANN_idir='E:\DATA\2.Satellite\PERSIANN-daily\BIN\';
odir='F:\GlobalTCMask1\single_pre\PERSIANN_Single_Precip3d\';
PERfilename='PERSIANN_CDR_AW05d_';
...
parfor NO=9357:13476 %注意年分1950开始
...
for k=1:length(allday)
    [~,~,allpre(:,:,k)] = read_ARCascii([PERSIANN_idir,'PERSIANN_CDR_AW05d_',num2str(floor(allday(k)/10000)),YMD_num(allday(k)),'.txt']); %读取出来的ERA5原数据经过重采样到标准网格
end
...
%OUTPUT(odir,['PERSIANNCDR_Ori_SingleTC_3dPrecip_Ummd','_SID',SID(:,NO)'],header,TCpre);%MM/DAY
...
output=[MP,MaxPreLON,MaxPreLAT,MaxPre1DLON,MaxPre1DLAT,Max10PreLON,Max10PreLAT,Max10Pre1DLON,Max10Pre1DLAT];
%delete(p);