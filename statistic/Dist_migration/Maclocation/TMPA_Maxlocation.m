...
odir='F:\GlobalTCMask1\single_pre\IRTPMA_Single_Precip3d_mmd\';
...
TOP25PreLAT=zeros(8171,1);%每场台风的最大降水强度---都是位置 
TOP25PreLON=zeros(8171,1);%每场台风的最大日降水强度
MP=zeros(8171,1);

parfor NO= 11089+60:13355 % 1998-2019
...
for k=1:length(allday)
    ...
    deficitdata025= ncread([TMPA_idir,'3B42_Daily.',STRdate,'.7.nc4'],'IRprecipitation');%读取每天的数据
    deficitdata025(deficitdata025<0)=0;%去除空值
    deficitdata025(isnan(deficitdata025))=0;%去除空值
...
OUTPUT(odir,['IRTMPA_3B42V7_Ori_SingleTC_3dPrecip_Ummd','_SID',SID(:,NO)'],header,TCpre);
