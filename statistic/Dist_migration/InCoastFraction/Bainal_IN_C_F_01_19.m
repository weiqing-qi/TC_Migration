...
Preindir='F:\GlobalTCMask1\single_tcw\ERA5_Single_TCW3dAVE\'; %改
Prefile=dir([Preindir,'*.txt']);
Prefile=Prefile(2399:4338);
%Prefile=Prefile(1:1940);%改2001开始!!!!!!!!!!!!IMERG
...
%按年求平均再线性回归
year1=xlsread('C:\Users\Dell\Desktop\Desktop2\Global_cyclone_project\DATA2.xlsx',2,'C2401:C4340');%改

R_PR_YAVE=zeros(19,28,7);%-800：2000km
for bas=1:7
    for yi=2001:2019
        Jud=JudgeB(:,bas);
        ...
        for DR=1:28
           ...
           R_PR_YAVE(yi-2000,DR,bas)=mean(R1C);
        end
    end
end
...
for bas=1:7
    for DInt=1:28
    [b,bint,r,rint,stats]=regress(R_PR_YAVE(:,DInt,bas),[ones(2019-2001+1,1),(2001:2019)'],0.05);
        PR_Regress(DInt,1,bas)=b(2);
        PR_Regress(DInt,2,bas)=bint(2,1);%注意
        PR_Regress(DInt,3,bas)=bint(2,2);
        PR_Regress(DInt,4,bas)=stats(3); 

    [ci_lower,ci_upper,pvals,b_Test,DW_p] = NeweyWestAdjust(r,(2001:2019)',R_PR_YAVE(:,DInt,bas),1,1);
     ...
     [Zs, p_value, UFk, UBk2, beta, beta_CI, corr_val]= MKtrend((2001:2019)',R_PR_YAVE(:,DInt,bas));
