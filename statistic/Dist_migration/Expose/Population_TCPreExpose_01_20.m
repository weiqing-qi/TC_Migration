...
%% 计算一年中最大降水强度分布和人口分布
indir='G:\DATA\GlobalTCMask1\single_pre\IMERG_Single_Precip3d_mmd\'; %改
MaxPre=zeros(360,720,20);
parfor y=2001:2020
    files=dir([indir,'IMERGV6_Ori_SingleTC_3dPrecip_Ummd_SID',num2str(y),'*.txt']);%MM/D注意单位 %改
...
%% ------------------计算人口
Popindir='G:\DATA\Landscan05_txt\';
PopDistri_TCP=zeros(360,720,20);%每年经历台风降水的人
PopAll_TCP=zeros(20,1);%每年总人数
PopRing_TCP=zeros(20,8);   %每环800-700...100-0km to coast
Pop_TCP_Basin=zeros(20,6);   %
PopRing_TCP_Basin=zeros(20,8,6);   %每环大洋800-700...100-0km to coast
...
PopDistri_TCP(:,:,y2-2000)=POPTCP;
PopAll_TCP(y2-2000)=sum(sum(POPTCP));
for basin1=1:6
Pop_TCP_Basin(y2-2000,basin1)=sum(POPTCP(all_in_one==basin1));
end
%------每个环里的人数
    for d=1:8    %800-700...100-0km to coast 
        ZoneLR=CRing_land(:,:,9-d);
        PopRing_TCP(y2-2000,d)=sum(POPTCP(ZoneLR==10000));   
        %-------分割大洋all_in_one
        LandRingPop=zeros(360,720);
        LandRingPop(ZoneLR==10000)=POPTCP(ZoneLR==10000);
        for basin2=1:6
            PopRing_TCP_Basin(y2-2000,d,basin2)=sum(LandRingPop(all_in_one==basin2)); 
        end
    end
end
...
for i=1:8
[b,bint,r,rint,stats]=regress(PopRing_TCP(:,i),[ones(2020-2001+1,1),(2001:2020)'],0.05);
    PR_Regress(i,1)=b(2);
