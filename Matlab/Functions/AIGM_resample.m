function [re_mask] = AIGM_resample(GWZ, incs, ocs)
% AIGM01_resample 用最邻近像元重采样一个AI掩膜
% GWZ = 90 90 -180 180 列优先掩膜索引 输出是 90 90 -180 180 二维数组
% rate>1 (downscale) 
% 目前最好ocs和incs能被180整除,不然采样会控制在之前范围内新精度最大能容纳的网格数
% 所以可能网格中心不会匹配
mask=zeros(180/incs,360/incs);
mask(GWZ)=1;
% 为输入栅格创建地理参考对象
latlim = [-90 90];  % 纬度限制
lonlim = [-180 180]; % 经度限制
