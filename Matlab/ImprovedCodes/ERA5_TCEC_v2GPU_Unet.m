%气旋经过网格周围气旋变量的值,NO，d循环中的读取会使得运行时间增加一百倍不止
clear; clc; close all;
    ...
    for hpa=950:-50:100
        ...
        for NO=1:10%NOlim(3)
            ...
            for d=1:length(allday)                                              %DayBF不要才能和lat数量对上，实际上是unique numtime，时间顺序应该是不会变的所以第一个和最后一个剪掉就行
                ...
                DailyIMERG=flipud(ncread([IMERG_idir,'3B-DAY.MS.MRG.3IMERG.',allday_S(d,:),'-S000000-E235959.V07B.nc4'],'precipitation'));
                [Buff_OWZ,Buff_OMask,Buff_IWZ,Buff_IMask,RMR] = TCLine2Buffer_GPU(WZ,Oradius,Iradius,cs,LonCT_gpu,LatCT_gpu,DailyIMERG,RMRflag,DRMW);
                
                %提取AI_Mask      
                DayMask=EventMasks(:,d);
                DayMask(DayMask==0)=[]; %MISSING VALUE=0
                DayMask(isnan(DayMask))=[];
                AI_OMask=AIGM_resample(DayMask, 0.1, cs);
                %--------------remove flash--------------
                ...
                if ~isempty(AIWZ)
                    D=ones(length(AIWZ),1)*10000;
                    for i=1:length(AIWZ)
                        TGCLON=LonCT_gpu(AIWZ(i)); 
                        TGCLAT=LatCT_gpu(AIWZ(i));
                        for j=length(londen)
                            CRadi = SphereDist2_Matrix([TGCLON,TGCLAT],[londen(j),latden(j)]); %每一列对应一个denlon位置
                            if CRadi<D(i)  D(i)= CRadi; end  %先求每个掩膜点到路径的最短距离然后把最大的D作为半径   
                        end
                    end
                    OradiusNEW=max(D);
                else 
                ...
                ORADIUUS(NO)= OradiusNEW;
                [~,Buff_OMask,~,~,~] = TCLine2Buffer_GPU(WZ,OradiusNEW,Iradius,cs,LonCT_gpu,LatCT_gpu,DailyIMERG,0,DRMW);
