...
%----------------------数据预设---------------------------------------------
% %----Python anaconda 环境
% pe = pyenv(Version="C:/ZS/setup/Anaconda/envs/ATTRI_GPU/python.exe",ExecutionMode="InProcess");
% % 添加脚本文件路径到Python的sys.path
% if count(py.sys.path,'D:\\DATA\\Segmentation map tutorial\\AImask.py') == 0
%     insert(py.sys.path,int32(0),'D:\\DATA\\Segmentation map tutorial\\AImask.py');
% end
% % 导入Python模块
%  py.AImask
% pyrunfile("AImask.py")
% [karen_images, TC_info] = py.AImask.process("karen",2001, "D:\\DATA\\Segmentation map tutorial\\Satellite images\\", ...
%                                    "D:\\DATA\\Segmentation map tutorial\\IBTrACS.ALL.v04r00.nc",11508);
%----降水数据
IMERG_idir='E:\DATA\2.Satellite\IMERG_Final_V7B_Daily\';
odir='D:\DATA\TC_spatial_data\IMERG_FV7B_PRE01_Unet\';
Outname='IMERGFV7B_TC_AVE_PreRate_Day01_Unetmask_Ummd';
Excel_file='D:\Desktop2\Attri-project\Results\Unet\Attri_Data_Unet.xlsx';
...
try
    ...
    for NO=ST:EN
        ...
        if S_AI_MASK==1
            alldata=zeros(180/cs,360/cs,length(allday));                        %bwconncomp不支持GPU输入
            allRB_Mask=zeros(180/cs,360/cs,length(allday),'gpuArray');          %每天只有雨带降水掩膜
            % allF_Mask=zeros(180/cs,360/cs,length(allday),'gpuArray');        %每天包含雨带全部降水掩膜,根据需不需要区分allO_Mask和allF_Mask来决定
        elseif S_AI_MASK==0
       ...
       %-------本场气旋所涉及降水数据读取----------------------------------------
        ...
        for d=1:length(allday)                                              %DayBF不要才能和lat数量对上，实际上是unique numtime，时间顺序应该是不会变的所以第一个和最后一个剪掉就行
            ...
            Iradius=100;
            [Buff_OWZ,Buff_OMask,Buff_IWZ,Buff_IMask,RMR] = TCLine2Buffer_GPU(WZ,Oradius,Iradius,cs,LonCT_gpu,LatCT_gpu,Dailydata,RMRflag,DRMW);
            ...
            if S_AI_MASK==1
                ...
                %-------AIMask覆盖降水为空则使用RB_Mask程序弥补
                %Percentage1(NO,d)=length(ismember(find(AI_OMask.*Dailydata>0),find(AI_OMask==1)))/length(find(AI_OMask==1));
                if sum(AI_OMask.*Dailydata,"all","omitmissing")==0
                    [ORing_Mask,AI_OMask,~] = OuterRainBelt(thres,Garea2D,Dailydata,Buff_OMask,Buff_OWZ,Buff_IWZ,cs);
                    AI_OMask=double(AI_OMask);%unit8不能和double.*
                end
            end                    
            %每天计算降水然后整体平均，并把整体的空间图存出来
            allI_Mask(:,:,d)=Buff_IMask;
            allO_Mask(:,:,d)=AI_OMask;
            allOR_Mask(:,:,d)=ORing_Mask;
            % if S_AI_MASK==1
            %     allRB_Mask(:,:,d)=RB_Mask;
            %     allF_Mask(:,:,d)=F_Mask;
            % end
            %--------当usawind>34,44NS,时候输出本场气旋当日数据--------
            if ~isempty(DWIND)
                if min(DWIND)>34 && max(Dlat)<=44 && min(Dlat)>=-44
                    DOUT=AI_OMask.*Dailydata;
                    if sum(DOUT,"all","omitmissing")~=0  %输出为空就不要输出了
                        DOUT(DOUT==0)=missing;
                        %Percentage2(NO,d)=length(ismember(find(AI_OMask.*Dailydata>0),find(AI_OMask==1)))/length(find(AI_OMask==1));
        ...
        TCD_MPCo_O(NO,:)  = [LonCT_gpu(rem(permute(I_MPO , [3,1,2])',180/cs*360/cs)),fillarray, ...
            LatCT_gpu(rem(permute(I_MPO , [3,1,2])',180/cs*360/cs)),fillarray]; %等价：LonCT_gpu3D(permute(I_MPO , [3,1,2]));
        TCD_MPCo_I(NO,:)  = [LonCT_gpu(rem(permute(I_MPI , [3,1,2])',180/cs*360/cs)),fillarray, ...
            LatCT_gpu(rem(permute(I_MPI , [3,1,2])',180/cs*360/cs)),fillarray];
        TCD_MPCo_OR(NO,:) = [LonCT_gpu(rem(permute(I_MPOR, [3,1,2])',180/cs*360/cs)),fillarray, ...
            LatCT_gpu(rem(permute(I_MPOR, [3,1,2])',180/cs*360/cs)),fillarray];
        TCD_MPCo_OL(NO,:) = [LonCT_gpu(rem(permute(I_MPOL, [3,1,2])',180/cs*360/cs)),fillarray, ...
            LatCT_gpu(rem(permute(I_MPOL, [3,1,2])',180/cs*360/cs)),fillarray];
        TCD_MPCo_OS(NO,:) = [LonCT_gpu(rem(permute(I_MPOS, [3,1,2])',180/cs*360/cs)),fillarray, ...
            LatCT_gpu(rem(permute(I_MPOS, [3,1,2])',180/cs*360/cs)),fillarray];
    ...
    xlswrite(Excel_file,OUT_PR(ST:EN,:)    ,1,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(EN+2)]);%改
    xlswrite(Excel_file,OUT_Am(ST:EN,:)    ,2,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(EN+2)]);%改
    xlswrite(Excel_file,OUT_MaxP(ST:EN,:)  ,3,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(EN+2)]);%改  
    xlswrite(Excel_file,OUT_PA(ST:EN,:)    ,4,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(EN+2)]);%改
    xlswrite(Excel_file,OUT_MPCo(ST:EN,:)  ,5,[num2abc2(5),num2str(ST+2),':',num2abc2(5+500-1),num2str(EN+2)]);%改
    xlswrite(Excel_file,OUT_Entir(ST:EN,:) ,6,[num2abc2(5),num2str(ST+2),':',num2abc2(5+50 -1),num2str(EN+2)]);%改

catch exception
    disp(['出错了: ', exception.message]);
    disp(NO);
    beep;
    ...
    xlswrite(Excel_file,OUT_PR(ST:NO-1,:)    ,1,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(NO-1+2)]);%改
    xlswrite(Excel_file,OUT_Am(ST:NO-1,:)    ,2,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(NO-1+2)]);%改
    xlswrite(Excel_file,OUT_MaxP(ST:NO-1,:)  ,3,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(NO-1+2)]);%改  
    xlswrite(Excel_file,OUT_PA(ST:NO-1,:)    ,4,[num2abc2(5),num2str(ST+2),':',num2abc2(5+250-1),num2str(NO-1+2)]);%改
    xlswrite(Excel_file,OUT_MPCo(ST:NO-1,:)  ,5,[num2abc2(5),num2str(ST+2),':',num2abc2(5+500-1),num2str(NO-1+2)]);%改
    xlswrite(Excel_file,OUT_Entir(ST:NO-1,:) ,6,[num2abc2(5),num2str(ST+2),':',num2abc2(5+50 -1),num2str(NO-1+2)]);%改
