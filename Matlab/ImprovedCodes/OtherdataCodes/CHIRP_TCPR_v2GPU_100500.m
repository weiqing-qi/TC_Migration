...
%----------------------数据预设---------------------------------------------
%----降水数据
CHIRP_idir='D:\DATA\CHIRP\TIFF\';
odir='D:\DATA\TC_spatial_data\pre_amount\CHIRP\';
Outname='CHIRP_TC_AVE_PreAmount_Day05_500km_Ummd';
%Excel_file='D:\Desktop2\Attri-project\Results\Otherdatasets\CHIRP_100500.xlsx';
...
RMRflag=0;                                                              %改
NOlim=lim_Y2NO([1981,2020],ISO_TIME);                                   %改
NODATA_value=-9999;                                                     %改
[LonCT,LatCT] = GridCenterLocation_GPU(cs);
[~,Garea2D] = Gridarea3D(cs);
[~,Garea005] = Gridarea3D(0.05);
%----陆地掩膜
...
for NO=ST:EN
    ...
    %-------本场气旋所涉及降水数据读取--------------------------------
    uniqueY = unique(allyear);
    % for j = 1:length(uniqueY)
    %     Yeardata = ncread([CHIRP_idir,'chirp.',num2str(uniqueY(j)),'.days_p05.nc'],'precip');
    %     Yeardata = permute(Yeardata,[2 1 3]);
    %     Yeardata = flipud(Yeardata);
    % 
    %     idx = find(allyear == uniqueY(j));  % 不能用year因为没有按照day unique
    %     for k = 1:length(idx)
    %         [Y, M, D] = datevec(datenum(allday_S, 'yyyymmdd'));  % 从日期数字中提取年、月、日
    %         Daydata = Yeardata(:, :, YMD_num(Y(idx(k)), M(idx(k)), D(idx(k))));
    %         alldata(:,:,idx(k)) = [zeros(40/cs,360/cs);Daydata;zeros(40/cs,360/cs)];%CHIRPS 50NS
    %     end
    % end
    % 
    % for k=1:length(allday)
    %     [~,Daynum] = YMD_numV2(allday(k));
    %     deficitdata005= ncread([CHIRP_idir,'chirp.',allday_S(k,1:4),'.days_p05.nc'],'precip',[1,1,Daynum],[inf,inf,1]);%读取每天的数据
    %     deficitdata005=flipud(deficitdata005');
    % 
    %     fulldata005=[repmat(NODATA_value,800,7200);deficitdata005;repmat(NODATA_value,800,7200)];
    %     resampled=areaweight_downscale(fulldata005, 0.05, cs, Graea005, Graea2D);
    %     alldata(:,:,k)=resampled;%上下反转，然后重采样
    % end
    for k=1:length(allday)
        deficitdata005 = imread([CHIRP_idir,sprintf('chirp.%04d.%02d.%02d.tif', ...
            str2double(allday_S(k,1:4)), str2double(allday_S(k,5:6)), str2double(allday_S(k,7:8)) )]);
        fulldata005=[repmat(NODATA_value,800,7200);deficitdata005;repmat(NODATA_value,800,7200)];
        resampled=areaweight_downscale(fulldata005, 0.05, cs, Garea005, Garea2D);
        alldata(:,:,k)=resampled; %读取出来tiff不需要翻转
    end
    alldata(alldata<0)=0;
    ...
    Spa_PAm(allO_Mask_FLAT==0)=missing;
