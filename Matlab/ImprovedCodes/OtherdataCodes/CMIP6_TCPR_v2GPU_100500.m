...
for model_name = allmodel_name
    if ismember(model_name, {'BCC-CSM2-MR'})%跳过的模型
        continue
    end
    CMIP6_idir=['D:\DATA\CMIP6\',model_name{:},'\pr\'];

    odir=['D:\DATA\TC_spatial_data\PreRate\CMIP6\',model_name{:},'\'];
    % 检查文件夹是否存在，如果不存在就创建
    if ~exist(odir, 'dir')
        mkdir(odir);
    end
    Outname=[model_name{:},'_TCAVE_PR_Day_500km_Ummd'];
    Excel_file=['D:\DATA\CMIP6\',model_name{:},'\',Outname,'_100500.xlsx'];
    % 检查文件是否存在，如果存在则删除
    ...
    %----气旋数据 这个数据如果有变化一定要清空所有的D:\DATA\TC_spatial_data\PreRate\CMIP6\ txt
    [LAT, LON, observation_time] = read_cmip6_track_list_nc(['D:\DATA\CMIP6\',model_name{:},'\',model_name{:},'_TC_FinalV2_tracks1980-2014.nc']);
    %----参数预设
    ...
    thres=0.1;                                                              %改 0.1 mm/h 2.4 mm/d 动态阈值不行啊
    RMRflag=0;                                                              %改
    NOlim=lim_Y2NO([1980,2014],observation_time);                           %改
    NODATA_value=-9999;                                                     %改
    [LonCT,LatCT] = GridCenterLocation_GPU(cs);
    [~,Garea2D] = Gridarea3D(cs);
    
    method = 'nearest'; %重采样方法
    ...
    % tic
    for NO=ST:EN
        if mod(NO, 100) == 0, fprintf('%s: %d\n', model_name{:}, NO); end
        
        time=observation_time(:,:,NO + NOlim(1)-1);
        lat=LAT(:,NO + NOlim(1)-1); 
        lat(isnan(lat) )=[];

        % if mean(abs(lat))>60
        %     error(model_name{:}, num2str(NO), '纬度过高');  
        % end 
        ...
        month  =str2num(time(5:6,1:length(lat))');                          %str2double只能对一个值使用
        day    =str2num(time(7:8,1:length(lat))');
        ...
        %-------本场气旋所涉及降水数据读取--------------------------------
        alldata = read_cmip6_pr(model_name{:}, allday_S, CMIP6_idir, cs, method, leap) * 86400; %单位是kg/m2/s需要转换
        alldata(alldata<0)=0;
        ...
        %剪裁输出空间降水图
        ZipView_ARCtxt('cut&save',Spa_PR,odir,[Outname,'_NO',num2str(NO),'_',allday_S(1,:)],cs,LatCT,LonCT,NODATA_value,2);
    ...
    %------------全周期结果计算--------------    
    for i = ST:EN
        time=observation_time(:,:,i + NOlim(1)-1);
        lat=LAT(:,i + NOlim(1)-1); 
        lat(isnan(lat) )=[];
        %-------每场气旋(每三小时)的日尺度时间点数值形式get-----------------------
        year   =str2num(time(1:4,1:length(lat))');                          %str2num可以对多行字符串使用变成一个数组
    ...
    % TCR_PA_O=Arr4; TCR_PA_L=Arr4; TCR_PA_S=Arr4;%真实影响面积
    %----------------结果输出到excel表格
    begin_year =NaN(NOlim(3),1);
    Basin =NaN(NOlim(3),1);
    for b = 1: NOlim(3)
        begin_year(b) = str2double(observation_time(1:4,1,b)');
    end
    TCR_PA=[TCR_PA_O,Arr4,Arr4,TCR_PA_L,TCR_PA_S,NaN(NOlim(3),4),begin_year];
    ...
    xlswrite(Excel_file,THead_PR     ,'PR',[num2abc2(3),num2str(1),':',num2abc2(5+250-1),num2str(2)]);%改
    xlswrite(Excel_file,THead_PAm    ,'PAm',[num2abc2(3),num2str(1),':',num2abc2(5+250-1),num2str(2)]);%改
    xlswrite(Excel_file,THead_MaxP   ,'MaxP',[num2abc2(3),num2str(1),':',num2abc2(5+250-1),num2str(2)]);%改  
    xlswrite(Excel_file,THead_PA     ,'PA',[num2abc2(3),num2str(1),':',num2abc2(5+250-1),num2str(2)]);%改
    xlswrite(Excel_file,THead_MPCo   ,'MPCo',[num2abc2(3),num2str(1),':',num2abc2(5+500-1),num2str(2)]);%改
    xlswrite(Excel_file,THead_Entir  ,'Entire',[num2abc2(3),num2str(1),':',num2abc2(5+55 -1),num2str(2)]); %改
