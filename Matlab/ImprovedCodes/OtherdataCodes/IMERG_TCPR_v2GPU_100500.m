...
odir='D:\DATA\TC_spatial_data\pre_amount\IMERG\';
% Outname='IMERGFV7B_TC_AVE_PreRate_Day01_500KM_Ummd';
Outname='IMERG_TC_PreAmount_Day05_500km_Um3';
% Excel_file='D:\Desktop2\Attri-project\Attri_Data.xlsx';
...
%----气旋数据
TC_idir='E:\DATA\5.TCdata\IBTrACS\IBTrACS.since1980.v04r00.20240122.nc';
[SID,LAT,LON,ISO_TIME,~,~,USA_WIND,~,~,~,~,~,~]=IBTrACS_nc_entire_variable_r(TC_idir);

%----参数预设
cs = 0.5;                                                               %改
S_RB_Buff=0;                                                            %改  是否追踪外雨带
...
for NO=1:NOlim(3)
    ...
    for k=1:length(allday)
        ...
        alldata(:,:,k)=areaweight_downscale(flipud(data), 0.1, cs);                                    %读取出来的IMERG数据需要翻转，具体见文件内
    end
    alldata(alldata<0)=0;
    ...
    for d=1:length(allday)                                              %DayBF不要才能和lat数量对上，实际上是unique numtime，时间顺序应该是不会变的所以第一个和最后一个剪掉就行
        ...
        Iradius=100;
        [Buff_OWZ,Buff_OMask,Buff_IWZ,Buff_IMask] = TCLine2Buffer_GPU(WZ,Oradius,Iradius,cs,LonCT,LatCT,Dailydata,RMRflag);

        if S_RB_Buff==1        
            [ORing_Mask,Buff_OMask,RB_Mask] = OuterRainBelt(thres,Garea2D,Dailydata,Buff_OMask,Buff_OWZ,Buff_IWZ,cs);
        elseif S_RB_Buff==0
            ORing_Mask=Buff_OMask;%S_RB_Buff==0和1时候ORing不同
...
xlswrite(Excel_file,OUT_PR    ,1,[num2abc2(5),num2str(3),':',num2abc2(5+250-1),num2str(3+NOlim(3)-1)]);%改
xlswrite(Excel_file,OUT_Am    ,2,[num2abc2(5),num2str(3),':',num2abc2(5+250-1),num2str(3+NOlim(3)-1)]);%改
xlswrite(Excel_file,OUT_MaxP  ,3,[num2abc2(5),num2str(3),':',num2abc2(5+250-1),num2str(3+NOlim(3)-1)]);%改  
xlswrite(Excel_file,OUT_PA    ,4,[num2abc2(5),num2str(3),':',num2abc2(5+250-1),num2str(3+NOlim(3)-1)]);%改
xlswrite(Excel_file,OUT_MPCo  ,5,[num2abc2(5),num2str(3),':',num2abc2(5+500-1),num2str(3+NOlim(3)-1)]);%改
xlswrite(Excel_file,OUT_Entir ,6,[num2abc2(5),num2str(3),':',num2abc2(5+50-1),num2str(3+NOlim(3)-1)]); %改
