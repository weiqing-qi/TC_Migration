...
%----------------------数据预设---------------------------------------------
%----降水数据
PERSIANN_idir='E:\DATA\2.Satellite\PERSIANN\';
odir='D:\DATA\TC_spatial_data\PreRate\PERSIANN\';
Outname='PERSIANN_TC_AVE_PreRate_Day025_500km_Ummd';
Excel_file='D:\Desktop2\Attri-project\Results\Otherdatasets\PERSIANN_100500.xlsx';
...
RMRflag=0;                                                              %改
NOlim=lim_Y2NO([2001,2019],ISO_TIME);                                   %改
NODATA_value=-9999;                                                     %改
...
for NO=ST:EN
    ...
    for k=1:length(allday)
        [STRD,~] = YMD_numV2(allday(k));
        if ismember(allday(k),[20131230,20131231,20140101,20140102,20140103,20140104])%这四天有问题
            alldata(:,:,k) = read_PERCDRbin([PERSIANN_idir,'\errordata20131230-20140104\','aB1_d',allday_S(k,3:4),STRD,'.bin'],cs);
        else
            alldata(:,:,k) = read_PERSIANNbin([PERSIANN_idir,'ms6s4_d',allday_S(k,3:4),STRD,'.bin'],cs);
        end
    end
    alldata(alldata<0)=0;
