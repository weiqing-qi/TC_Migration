...
%----------------------数据预设---------------------------------------------
%----降水数据
MSWEP_idir='E:\DATA\4.Merge\MSWEP\';
odir='D:\DATA\TC_spatial_data\pre_amount\MSWEP\';
% Outname='MSWEP_TC_AVE_PreRate_Day05_500KM_Ummd';
Outname='MSWEP_TC_PreAmount_Day05_500KM_Um3';
% Excel_file='D:\Desktop2\Attri-project\Results\Otherdatasets\MSWEP_100500.xlsx';
...
for NO=1:NOlim(3)
    ...
    for k=1:length(allday)
        if allyear(k)<=2016
            Adaydata = ncread([MSWEP_idir,'MSWEP\MSWEP\',allday_S(k,1:6),'.nc'],'precipitation',[1,1,str2double(allday_S(k,7:8))],[inf,inf,1]);%每一个allday对应一个allpre
            alldata(:,:,k)=Adaydata'; %读取出来的MSWEP重采样到标准网格,转置
        elseif allyear(k)>2016 && allyear(k)<=2020
            [Daystr,~] = YMD_numV2(allday(k));    
            Adaydata = ncread([MSWEP_idir,allyear_S(k,:),'\',allyear_S(k,:),Daystr,'.nc'],'precipitation');%每一个allday对应一个allpre
            Adaydata=areaweight_downscale(Adaydata', 0.1, cs, Garea01, Garea2D);
            alldata(:,:,k)=Adaydata;
        end
    end
    alldata(alldata<0)=0;
...
% TCR_PA_O=Arr4; TCR_PA_L=Arr4; TCR_PA_S=Arr4;%真实影响面积
%----------------结果输出到excel表格
TCR_PA=[TCR_PA_O,Arr4,Arr4,TCR_PA_L,TCR_PA_S,NaN(NOlim(3),5)];
