...
odir='D:\DATA\TC_spatial_data\pre_amount\ERA5\';
Outname='ERA5_TC_AVE_PreAmount_Day05_500km_Ummd';
% Excel_file='D:\Desktop2\Attri-project\Results\Otherdatasets\ERA5_100500.xlsx';
...
for NO=ST:EN
    ...
    for m=1:length(allmon)
        Amonthdata = ncread( ...
            [ERA5_idir,'ERA5_Total_precipitation_on_single_levels_daily',allmon_S(m,:),'.nc'],'tprate');% 改
        ...
        for k=1+add:length(mon_d)+add
            ...
            alldata(:,:,k)=flipud(Amonthdata(:,2:180/cs+1,str2double(allday_S(k,7:8)))')*1000*86400;    %Unit: MM/D 
        end                          
