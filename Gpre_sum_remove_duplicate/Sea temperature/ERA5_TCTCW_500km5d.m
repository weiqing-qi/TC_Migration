...
ERA5_idir='E:\DATA\ERA5-Total-Column-Water\';
...
parfor NO=13216:13476      %5306:13476 %注意年分1950开始
...
for k=1:length(allday)
    ...
    Amonthdata = ncread([ERA5_idir,ERAfilename,STRdate(1:6),'.nc'],'tcw');%每一个allday对应一个allpre 2003.7.31有问题
    if allday(k)==20030731
        allpre(:,:,k)=ERA5dailyresample( flipud(Amonthdata(:,:,30)') ,Garea025);
        continue
    end
    allpre(:,:,k)=ERA5dailyresample( flipud(Amonthdata(:,:,str2double(STRdate(7:8)))') ,Garea025); %读取出来的ERA5原数据经过重采样到标准网格
end
