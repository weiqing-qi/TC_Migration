...
odir='F:\GlobalTCMask1\NODEduplicate_sum\ERA5origin\';
Sodir='F:\GlobalTCMask1\NODEduplicate_sum\ERA5origin\ERA5seasonOri\';
...
for k=1:length(allday)
    ...
    allpre(:,:,k)=ERA5dailyresample( flipud(Amonthdata(:,:,str2double(STRdate(7:8)))')*1000*86400 ,Garea); %读取出来的ERA5原数据经过重采样到标准网格
end
