...
odir='F:\GlobalTCMask1\single_SST\ERA5_Pre3d_sst\';
...
S_TC_Pre=zeros(8171,1);%每场台风的陆地降水 
S_Land_Pre=zeros(8171,1);%每场台风的陆地降水
MaximumPre=zeros(8171,1);%每场台风的最大降水
MaximumLPre=zeros(8171,1);%每场台风的最大陆地降水
MaximumPre10grid=zeros(8171,1);%每场台风的最大降水
MaximumLPre10grid=zeros(8171,1);%每场台风的最大陆地降水

parfor NO=7064:13476 %注意年分1966开始
...
numtime=zeros(length(lat),1);
  ...
  for i=1:length(lat)
    ...
    year(i)=cellstr(TDA(1:4));%将字符数组转换为cell型字符串数组:字符数组S中的每行分割成为cell细胞元组C的一个元素,并删除S的每行尾部空格
    month(i)=cellstr(TDA(5:6));
    day(i)=cellstr(TDA(7:8));
    numtime(i,1)=str2double(TDA);%以数字形式储存时间方便去重
  end
...
for k=1:length(allday)
    ...
    allpre(:,:,k)=ERA5dailyresample( flipud(Amonthdata(:,:,str2double(STRdate(7:8)))')-273.15 ,Garea025); %读取出来的ERA5原数据经过重采样到标准网格
end
...
OUTPUT(odir,['ERA5_PRE3d_Ori_SingleTC_SST','_SID',SID(:,NO)'],header,TCpre);
...
Pcurrent(Pcurrent<-50)=0;%nonvalue变成0可以求和
...
S_TC_Pre(NO-5305)=sum(sum(Pcurrent.*Garea05))/sum(sum(Garea05(Pcurrent>0)));%每场台风的平均海温
