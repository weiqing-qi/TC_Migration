%每一场台风的降水量MSWEP2017-2020   0-360注意
%update20201020
clear; clc; close all;
header=['ncols           720';
        'nrows           360';
...
parfor NO=13021:13476 %注意年分1950开始
...
for k=1:length(allday)
    ...
    Adaydata = ncread([MSWEP_idir,char(year(1)),'\',char(year(1)),STRdate,'.nc'],'precipitation');%每一个allday对应一个allpre
    allpre(:,:,k)=areaweight01_05(Adaydata'); %读取出来的MSWEP重采样到标准网格,转置
end
...
%process 4 calculate Precipitation of each grid 
  for j=1:(length(weizhi))
     ...
     NOdataexclute=zeros(5,1);
      ...
      TCpre(weizhi(j)) = sum(NOdataexclute);%0-360，位置不变，注意
  end
  
OUTPUT(odir,['pMSWEP',num2str(NO-9017,'%04d'),'_SID',SID(:,NO)'],header,TCpre);%为了和80年开始的记录连上所以-9017
...
S_TC_Pre(NO-13020)=10^(-6)*sum(sum(Pcurrent.*Garea05));%每场台风的降水量 十亿m3
S_Land_Pre(NO-13020)=10^(-6)*sum(sum(LPcurrent.*Garea05));%每场台风的陆地降水量 十亿m3
MaximumPre(NO-13020)=max(max(Pcurrent));%每场台风的最大降水强度 0.5°网格mm/5d
MaximumLPre(NO-13020)=max(max(LPcurrent));%每场台风的陆地最大降水强度 0.5°网格mm/5d
MaximumPre10grid(NO-13020)=mean(OnecolumnP(1:10));%每场台风的最大10降水强度0.5°网格mm/5d
MaximumLPre10grid(NO-13020)=mean(OnecolumnLP(1:10));%每场台风的最大10陆地降水强度0.5°网格mm/5d
end
