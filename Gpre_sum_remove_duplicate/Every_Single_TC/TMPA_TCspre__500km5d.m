%每场台风500km影响区降水总量 前后五天相加的版本  FOR TMPAV7
%update20220403 注意这个没有去除降水数据中的nodata value因为mswep没有坏点  跨年台风的降水会算进前一年  
clear; clc; close all;
header=['ncols           720';
        'nrows           360';
        'xllcorner      -180';
...
TMPA_idir='E:\TMPA_3B42_V7_Daily\';
odir='F:\GlobalTCMask1\single_pre\TMPA\';
...
parfor NO= 11089:13355 % 1998-1999年第一场开始
...
for k=1:length(allday)
    ...
    deficitdata025= ncread([TMPA_idir,'3B42_Daily.',STRdate,'.7.nc4'],'precipitation');%读取每天的数据
    deficitdata025(deficitdata025<0)=0;%去除空值
    fulldata025=[zeros(160,1440);deficitdata025;zeros(160,1440)];
    allpre(:,:,k)=areaweight025_05(flipud(fulldata025));%上下反转，然后重采样
end
...
%process 4 calculate Precipitation of each grid 
  for j=1:(length(weizhi))
      ...
      TCpre(locationR(j),locationC(j)) = sum(NOdataexclute);%-180180
  end

  OUTPUT(odir,['TMPA_3B42_V7_Ori_SingleTC_pre','_SID',SID(:,NO)'],header,TCpre);
...
S_TC_Pre(NO-11088)=10^(-6)*sum(sum(Pcurrent.*Garea05));%每场台风的降水量 十亿m3
S_Land_Pre(NO-11088)=10^(-6)*sum(sum(LPcurrent.*Garea05));%每场台风的陆地降水量 十亿m3
MaximumPre(NO-11088)=max(max(Pcurrent));%每场台风的最大降水强度 0.5°网格mm/5d
MaximumLPre(NO-11088)=max(max(LPcurrent));%每场台风的陆地最大降水强度 0.5°网格mm/5d
MaximumPre10grid(NO-11088)=mean(OnecolumnP(1:10));%每场台风的最大10降水强度0.5°网格mm/5d
MaximumLPre10grid(NO-11088)=mean(OnecolumnLP(1:10));%每场台风的最大10陆地降水强度0.5°网格mm/5d
area=sum(sum(Garea05(Pcurrent>1)));
t=3*length(lat);
PreRate(NO-11088)=S_TC_Pre(NO-11088)*10^6/area/t;
end
