...
MERRA2_idir='F:\MERRA2\MERRA2UCDaily05\';
odir='F:\GlobalTCMask1\NODEduplicate_sum\MERRA2Ori\';
Sodir='F:\GlobalTCMask1\NODEduplicate_sum\MERRA2Ori\MERRA2season\';
%MERRAfilename='MERRA2_PRECTOTCORR_pre_OriAW05_Daily';
MERRAfilename='MERRA2_PRECTOT_pre_OriAW05_Daily';
...
%把每一年的总降水和40年的整体降水输出
  ...
  OUTPUT(odir,['ERA5_GSUMTCP_Origin',num2str(Y)],header,outO);
  UntilLastyearG = G_SUM_PRE;
...
if str2double(char(month(1))) == 6 && S1==0
   ...
   OUTPUT(Sodir,['ERA5_GSUMTCP_Origin',char(year(1)),'_S1'],header,SoutO); 
...
if str2double(char(month(1))) == 9 && S2==0
   Sout = G_SUM_PRE - sensonprebase;
   SoutO=[Sout(:,(180/0.5)+1:360/0.5),Sout(:,1:180/0.5)];%输出-180-180
   OUTPUT(Sodir,['ERA5_GSUMTCP_Origin',char(year(1)),'_S2'],header,SoutO);
   sensonprebase= G_SUM_PRE;
   S2=1; 
end
if str2double(char(month(1))) == 12 && S3==0
   Sout = G_SUM_PRE - sensonprebase;
   SoutO=[Sout(:,(180/0.5)+1:360/0.5),Sout(:,1:180/0.5)];%输出-180-180
   OUTPUT(Sodir,['ERA5_GSUMTCP_Origin',char(year(1)),'_S3'],header,SoutO);
   sensonprebase= G_SUM_PRE;
   S3=1;    
end
if str2double(char(month(1))) == 3 && S4==0
   Sout = G_SUM_PRE - sensonprebase;
   SoutO=[Sout(:,(180/0.5)+1:360/0.5),Sout(:,1:180/0.5)];%输出-180-180
   OUTPUT(Sodir,['ERA5_GSUMTCP_Origin',num2str(Y-1),'_S4'],header,SoutO);
   sensonprebase= G_SUM_PRE;
   S4=1;    
end
            
%读acrtxt头文件和掩膜 %header   %1--ncols  %2--nrows  %3--xllcorner  %4--yllcorner  %5--cellsize  %6--NODATA_value
[~,inputheader,ras]= read_ARCascii ([files_idir,raster_files(NO-9017).name]); %注意文件名虽然不连续但是时间顺序是连续的就不会影响
...
for k=1:length(allday)
    ...
    rawdata = ncread([MERRA2_idir,MERRAfilename,STRdate(1:8),'.nc4'],'totalpre');%每一个allday对应一个allpre
    allpre(:,:,k)=flipud(rawdata'); %读取出来的ERA5原数据经过重采样到标准网格
end
